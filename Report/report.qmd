...
author:
  - name: J. Dante Maurice
    url: https://www.linkedin.com/in/joseph-dante-maurice-68a56526a/
  - name: Tai Fowler
    url: https://www.linkedin.com/in/tai-fowler-277573257/
date: "July 25, 2025"
toc: true
format:
  html:
...
data_baseballsavant_2024 = read_csv("data/savant_data_2024.csv")
...
Although we briefly considered applying Principal Component Analysis (PCA) to reduce dimensionality before clustering, we found that doing so did not substantially alter the clusters or resulting visualization. Ultimately, we retained the raw swing features to preserve interpretability.


## Results

We applied GMM clustering on the five swing metrics to each MLB hitter. Each hitter has their own unique clustering, with each cluster containing a distinct region of a hitter’s swing distribution. We consider a hitter’s “best” cluster(s) to be the ones with both a high contact rate and a high proportion being squared up and blasts (to quantify quality of contact). Below, you can see how these factors vary by cluster for Jose Altuve
...
Looking at the table, it is clear that Altuve’s best results mostly come from swings in the third cluster. Cluster 3 has the highest percentages across the board, and it is also the cluster with the largest population. This makes sense, as Altuve is an elite player who should be getting off his “A” swing a large majority of the time. 
...
Looking at a few of these plots, a notable trend emerged: hitters tend to use “worse” swings in two-strike counts compared to other situations. Below, you can see how contact and quality of contact vary by cluster for each of the players above.
...
An interesting application of swing clustering is the creation of batter hot and cold zones using swing cluster as a proxy for swing quality. Because swing quality can be influenced by both pitch type and location, we can use a multinomial model to predict swing cluster based on those features. By generating a prediction grid across the strike zone, we obtain hitter-specific hot and cold zones based on swing quality independent of the outcome of the swing (e.g. contact or batting average). Below are examples of these zones for José Altuve.
...
#| fig-height: 12
#| fig-width: 16
knitr::include_graphics("plots/altuve_fastballs.png")
```
...
#| fig-height: 12
#| fig-width: 16
knitr::include_graphics("plots/altuve_curveball.png")
```
...
```{r}
#| echo: false
results=read_csv("data/chi_square_results.csv")
...
```{r}
#| fig-cap: "This is a density plot of each tested player's p-value. The green dotted line is the standard 0.05 rejection value. The red dashed line is the Bonferroni corrected rejection value of about 0.00016"
...
While it is interesting to look at the hitters who do display significant discrepancies in their swing distribution in different strike situations (such as the hitters above), it is also good to look at hitters who don't. One such hitter is Luis Arraez, known for his high contact percentage and low strikeout rate. 

```{r}
#| echo: false
arraez_plots <- plot_player_clusters(swing_data_2024, "Arraez, Luis")
```
...
arraez_plots$contact_by_cluster %>%
  select(
    cluster,
    n,
    contact_pct,
  ...
  gt() %>%
  tab_header(
    title = "Arraez Contact Quality by Cluster"
  ) %>%
  cols_label(
    cluster = "Cluster",
    n = "Number of Swings",
...
For a hitter with a profile like Arraez, it makes sense that his swing distribution doesn't significantly vary when he has two strikes. Arraez continues to draw from the better part of his swing distribution in disadvantageous counts, which allows him to not only avoid strikeouts but also provide quality contact that supports his high average.
...
```{r}
#| echo: false
plot_player_clusters2 <- function(data, player_name) {
  library(mclust)
  library(ggplot2)
  library(dplyr)
  library(ggmosaic)
  ...
  plot8 <- ggplot(player %>%
    filter(!is.na(cluster), !is.na(strikes))) +
    geom_mosaic(aes(x = product(cluster, strike_group), fill = cluster)) +
    labs(title = paste0(name_title, " 2024 - Cluster vs. Strikes"),
         x = "Cluster × Strike Group",
         y = "Proportion") +
    theme_minimal()+
    theme(
...
```{r}
#| echo: false
stowers_plots <- plot_player_clusters2(swing_data_2024, "Stowers, Kyle")
```
...
```{r}
#| echo: false
data_baseballsavant_2025 = read_csv("data/savant_data_2025.csv")
...
player <- player %>%
  ...
  plot8 <- ggplot(player %>%
    filter(!is.na(cluster), !is.na(strikes))) +
    geom_mosaic(aes(x = product(cluster, strike_group), fill = cluster)) +
    labs(title = paste0(name_title, " 2025 - Cluster vs. Strikes"),
         x = "Cluster × Strike Group",
         y = "Proportion") +
    theme_minimal()+
    theme(
...
**Email:** jdantemaurice@gmail.com

**Tai Fowler**

**Phone:** (248) 954 - 9303

**Email:** taichingeagle44@gmail.com
